...
public:
     ...
     Array(QString name,Type typecontaint,int size):Type(name){this->setTypecontaint(typecontaint);this->setSize(size);}
     void setSize(int size){this->size = size;}
     void setTypecontaint(Type containtType){this->containtType = containtType;}
     int getSize(){return this->size;}
     Type getTypecontaint(){return this->containtType;}
     TypeBehavior Is(){return ARRAY;}
     std::string toString(){
         std::string tt = "";
         for(int i = 0; i < this->containtType.getMemoryRange();i++)
             tt += "@";
         return "\n\t" + this->getName().toStdString() + " = ARRAY[1.." + QString::number(size).toStdString() + "] OF " + tt + containtType.getName().toStdString();
     }
private:
    int size;
    Type containtType;
...
public:
     ...
     MatrixType(QString name,int columnsCount,int rowsCount,Type containtType):Type(name){setColumnsCount(columnsCount);setRowsCount(rowsCount);setTypecontaint(containtType);}
     ...
     std::string toString(){
         ...
         return "\n\t" + this->getName().toStdString() + " = ARRAY[1.." + QString::number(rowsCount).toStdString() + ",1.." + QString::number(columnsCount).toStdString() + " ] OF " + tt + containtType.getName().toStdString();
     }
...
public:
    ...
    FileType(QString name,Type typecontaint):Type(name){this->Typecontaint = typecontaint;}
    Type getTypecontaint(){return this->Typecontaint;}
    void setTypecontaint(Type typecontaint){this->Typecontaint = typecontaint;}
...
public:
    ...
    QalifierType(QString name,Type typeQalified):Type(name){this->typeQalified = typeQalified;}
    Type getTypeQalified(){return this->typeQalified;}
    void setTypeQalified(Type typeQalified){this->typeQalified = typeQalified;}
...
public:
    Variable(){}
    Variable(QString name,Type type):ObjectAlgo(name){setType(type);}
...
public:
    Argment(){}
    Argment(QString name,Type type,IOArgment io):Variable(name,type){this->io = io;}
