...
std::string DebugAnalyseExpression(Expression * expression)
{
    std::string str = "";
    ...
    try
    {
        while(expression)
        {
            ...
            {
                if(expression->hasExpressionOperator())
                    str += "(" + DebugAnalyseExpression(expression->getExpressionOperator()->FirstOperand) + ")";
                else
                    qDebug()<< "Parentese expression error no operator expression ";
            }
...
int main(int argc , char ** argv[])
{
    ...
    try
    {
        ...
        if(argc > 1)
        {
         ...
         try{
             a.setStream(expression);
             Expression * exp = s.AnalyseExpression(&a);
