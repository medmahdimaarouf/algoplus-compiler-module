    ...
    while (t) {
        ...
        ReloadAll:{
            ...
            if (!IN_Expression && !IN_ExpressionOperator)
            {
                ...
                {
                    expression = AnalyseExpression(lex);
                    t = previeusToken;
                    if(t->Nature() != Rp)
                        Error(t,"')' attendu");
                    else
                ...
                ExpressionOperator:{
                    ...
                    if(listExOPerators.contains(t->Value()) )
                    {
                        if(dynamic_cast<Expression*>(expression))
                        {
                            ...
                            exp2->setExpressionOperator(operatorExp);
                            return exp2;
                        }
                        else
                        if(!expression->hasExpressionOperator())
                        {
                            ...
                            Error(t," Duplication d 'operateur d 'expression " + expression->getExpressionOperator()->value->Value() + ":new " + t->Value());
                    }
                    else
                    {
                        EndExpression:{
...
Reference *SyntaxicAnalyser::GetReference(LexicalAnalyser *lex)
{
    ...
    while (t) {
        ReloadAll:{
            if(previeusToken->getLineNumber() != t->getLineNumber() || t->Nature() == Rp)
            {
                previeusToken = t;
                goto EndReference;
            }
