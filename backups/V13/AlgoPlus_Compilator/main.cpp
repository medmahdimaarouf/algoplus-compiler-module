...
std::string ReadStream(string file_name){

    if(file_name.empty())perror("NO FILE SPECIFIED");
    ...
    if(file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        {
            lexicalAnalyser.setStream(expression.toStdString());
        }
...
void LexicalAnalyse(){
    ...
    Token * token;
        ...
        lexicalAnalyser.BackToken();
        while(token = lexicalAnalyser.BackToken())cout<<token->toString()<<endl;
...
void AnalyseExpression(std::string exp_str){
    try{
        ...
        cout << " Pattern expression  : "<< exp_str <<endl<<"\n Analysed expression -> "<< exp->toString()<<endl<<" Current Token : " << lexicalAnalyser.getCurrentToken()->Value();
