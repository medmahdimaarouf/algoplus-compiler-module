...
StringList listAlgoKeyWords;
StringList listJsKeyWords;
...
Token * LexicalAnalyser::NextToken()
{
 ...
 // --
        ...
        case    '*' :
        case    '/' :{
            token->setValue(QString(stream[currentIndex]).toStdString());
...
// --
        ...
        default:
            if(isDigit(stream[currentIndex]))
            {
                NumberT:
                    {
                        bool reel;
                ...
                if(isLetter(stream[currentIndex]) || stream[currentIndex] == '_')
                {
                    token->setValue(Id());
                    if(std::find(listAlgoKeyWords.begin(),listAlgoKeyWords.end(),toUpper(token->Value())) != listAlgoKeyWords.end())
                        token->setNature(ID);
                    else
...
std::string LexicalAnalyser::Number(bool *reel)
{
    std::string number = "";
    *reel = false;
    while (currentIndex != -1 && isDigit(stream[currentIndex])) {
...
{
    std::string id = "";
    while (currentIndex != -1 && (isLetterOrNumber(this->stream[currentIndex]) || this->stream[currentIndex] == '_')) {
...
std::string LexicalAnalyser::lookAhead(const int nbc)const
{
    /*int i = 0;
    for(i = 1; i <= nbc && currentIndex + i < stream.size();i++);*/
    return stream.substr(currentIndex,nbc);

}

bool LexicalAnalyser::isIdToken(const std::string token) {

    if(token.empty())return false;
    if(!isLetter(token[0]) && token[token.size()-1] != '_' || count(' ',token) != 0)
        return false;
...
std::string LexicalAnalyser::setStream(const std::string stream){
