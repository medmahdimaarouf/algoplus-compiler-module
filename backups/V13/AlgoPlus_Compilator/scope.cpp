...
Scope::Scope(std::string name):ObjectAlgo(name)
{

}

Constant Scope::getConstant(std::string name)
{
    if(hasConstant(name))
...
Type & Scope::getType(std::string name)
{
...
TypeOfContaint Scope::getTypeContaint(std::string name)
{
    if(hasType(name))
...
void Scope::AddConstant(Constant constant)throw()
{
    ...
    if(listConstants->count(constant.getName()))
       ...
       listConstants->insert(std::make_pair<std::string,Constant>(constant.getName(),constant));
}
...
void Scope::AddVariable(Variable var)
{
    ...
    if(hasVariable(var.getName()))
        ...
        this->listVariables->insert(std::pair<std::string,Variable>(var.getName(),var));
}
...
void Scope::CheckExistID(std::string id)
{
    ...
    if(this->hasConstant(toUpper(id)))
        throw "'" + id + "' est un constante deja declare";
    else
    if(this->hasType(toUpper(id)))
        throw "'" + id + "' est un type deja declare";
    else
    if(this->hasVariable(toUpper(id)))
...
std::string Scope::toString()
{
    ...
    for(MapTypes::iterator i = listTypes->begin(); i != listTypes->end();i++)
    {
        ...
        {
            //desc +="NO casting" + T->toString()+ "to string typr id " + QString(typeid(*T).name()) +"\n";
            desc += T->toString() + "\n";
        }
    }
    desc += "\n VARIABLES \n";
