...
{       
    ...
    Swith_Statment * swith_stat = NULL;
        Case_Statment * case_stat = new Case_Statment();
    ...
    ReloadAll:{
            ...
            if(IN_Pour)
            {
                for_inst:{
                   if(IN_Compter)
                   {
                        NodeExpression * compter = AnalyseExpression(lex,false);
                        if(compter == NULL)
                   ...
                   if(IN_Vi)
                   {
                       NodeExpression * vi = AnalyseExpression(lex,false);
                       if(vi == NULL)
                   ...
                   if(IN_Vf)
                   {
                       NodeExpression * vf = AnalyseExpression(lex,false);
                       if(vf == NULL)
                           Error(t,"Valuer initial du boucle FOR attendu ");
                       else
                       {
                   ...
                   {
                       NodeExpression * stepValue = AnalyseExpression(lex,false);
            ...
            if (IN_TanteQue)
            {
                while_inst:{
                    if(IN_Expression)
                    {
                        NodeExpression * condition = AnalyseExpression(lex,false);
                        if(condition == NULL)
            ...
            if (IN_Selon)
            {
                switch_stat:{
                    if(IN_Selector)
                    {
                        NodeExpression * selector = AnalyseExpression(lex,false);
                        if(!selector)
...
NodeExpression *SyntaxicAnalyser::AnalyseExpression(LexicalAnalyser *lex,bool IN_LP)
{
    /**
    ...
    while (t) {
        ...
        ReloadAll:{
            ...
            if (!IN_Expression && !IN_ExpressionOperator && !IN_ExpressionWithSignelOp)
            {
                ...
                {
                    NodeExpression * ndr = AnalyseExpression(lex,true);
                    if(ndr == NULL)
                ...
                ExpressionOperator:{
                    ...
                    if(listExOPerators.contains(toUpper(t->Value())))
                    {
                        if(!expression->hasExpressionOperator())
                        {
                            ...
                            operatorExp->SecondOperand = AnalyseExpression(lex,false);
                ...
                _Expression:{
                    ...
                    if (!IN_reference && !IN_CallFunction && !IN_ConstValue)
                    {
                        ...
                        if(listExOPerators.contains(t->Value()))
                        {
                            ...
                            Error(t,"Expression illegal : Le mot '" + t->Value() + "' n est pas un facteur d'opertauer ");
                    }
                    else
                    if (IN_reference)
                    {
                    ...
                    if (IN_CallFunction)
                    {
                        CallFunction:{
                            ...
                            if(IN_EffectefArgs)
                            {
                                ...
                                {
                                    do{
                                        NodeExpression * arg = AnalyseExpression(lex,false);
                                        if(arg != NULL)
                                        {
            ...
            if(IN_ExpressionWithSignelOp)
            {
                ExpressionWithSignelOp:{
                    Token * opval = t;
                    NodeExpression * node = AnalyseExpression(lex,false);
                    if(node)
                    {
