...
void SyntaxicAnalyser::Instructions(Statment * parentstatment,LexicalAnalyser * lex)
{
    ...
    ReloadAll:{
            ...
            if(IN_Si)
            {
               ...
               if(IN_Expression)
               {
                   NodeExpression * cond = AnalyseExpression(lex);
                   t = endToken;
                ...
                repeat_inst:{
                    if(IN_Until)
                    {
                        if(1)//toUpper(t->Value()) != Jusqu_A)
                        {
                            ...
                            Error(t,"Mot cle 'JUSQU_A' attendu ");
                    }
                    else
                    {
...
{
    /**
     *  SI L EXPRESSION EST PARENTESE FIRST OP ARG DOIT ETRE UNE (EXPRESSION) ET SECONF ARG UNE SIMPE EXP
     **/
    bool IN_Expression              = false;
    ...
    Reference::ByteOperator * bop = NULL;
            ...
            if (!IN_Expression && !IN_ExpressionOperator && !IN_ExpressionWithSignelOp)
            {
                if(t->Nature() == Lp && expression == NULL)
                {
                    ...
                    else
                    {
                        ...
                        // + FOR CHECKING
                        if(endToken){
                            if(endToken->Nature() != Rp)
                                Error(endToken,"Expression illegal : Spearateur ' ) ' attendu " );
                        }
                        else
                ...
                _Expression:{
                    ...
                    if (!IN_reference && !IN_CallFunction && !IN_ConstValue)
                    {
                        ...
                        if(listExOPerators.contains(t->Value()))
                        {
                            ...
                            Error(t,"Expression illegal : Le mot '" + toUpper(t->Value()) + "' n est pas un facteur d'opertauer ");
                    }
                    else
                    if (IN_reference)
                    {
                        _Reference:{
                            ...
                            if(IN_ByteOperator)
                            {
                                if(t->Value() == ".")
                                {
                                    Token * member = lex->NextToken();
                                    if(member)
                                    {
                                        if(member->Nature() == ID)
                                        {
                                            Reference::ByteOperator * op = new Reference::ByteOperator();
                                            op->value = t;
                                            //op->Argmant = member;
                                        }
                                        else
                                            Error(member,"Expression illegal : identificateur membre attendu ");
                                    }
                                    else
                                        Error(t,"Expression illegal : Argument d'operateur bite '.' attendu");
                                }
                                else
                                if(t->Nature() == RI)
                                {
                                    ...
                                    if(arg == false)
                                        Error(t,"Expression illegale : Valeur d 'indexage attendu [ v ] ");
                                    else
                                    if(endToken)
                                    {
                                        if(endToken->Nature() == LI)
                                        {
                                            t = endToken;
                                           Reference::ByteOperator * op = new Reference::ByteOperator();
                                           op->Argmant = arg;
                                           ...
                                           bop = op;
                                            ...
                                            Error(endToken,"Expression illegale : ']' attendu ");
                                    }
                                    else
                                        Error(t,"Expression illegale : ']' attendu au fin ");
                                }
                                else
                                if(t->Value() == "@")
                                {
                            ...
                            if(IN_IDref)
                            {
                                if(t->Nature() == ID)
                                {
                                    expression = new Reference();
                                    dynamic_cast<Reference*>(expression)->setValue(t);
                                    IN_IDref = false;
                        ...
                        CallFunction:{
                            ...
                            if(IN_Rp)
                            {
                                if(endToken)
                                {
                                    ...
                                    if(dynamic_cast<FctCall*>(expression)->hasEffectArgs() && endToken->Value() == ",")
                                        Error(endToken,"Expression illegal : Un argument effective requis ");
                                    else
                                        Error(endToken,"Expression illegal : ')' attendu ");
                                ...
                                ListEfArgs:
                                {
                                    do{
                                        ...
                                        if(dynamic_cast<FctCall*>(expression)->hasEffectArgs())
                                            Error(endToken,"Expression illegal : Argument effective requis ");
                                        else
                                        {

                                        }
                                    }while(endToken->Value() == ",");
