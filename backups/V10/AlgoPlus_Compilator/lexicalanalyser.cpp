...
LexicalAnalyser::LexicalAnalyser()
{
    ...
    listKeys<<"ALGORITHME"<<"POUR"<<"SI"<<"SELON"<<"TANTEQUE"<<"REPETER"<<"JUSQU_A"<<"FN"<<"NON"<<"ET"<<"OU"<<"MOD"<<"DIV"<<"NIL"<<"NULLE"<<"FIN"<<"DANS"<<"VRAI"<<"FAUX"<<"RETOURNER";
    // - ATTENTION !
}
...
Token * LexicalAnalyser::NextToken()
{
 ...
 // --
        case    '<' :if( lookAhead(3) == "<--"){t->setNature(AssignValue);t->setValue(Advance(3)); break;  }
              else   if( lookAhead(2) == "<>"){t->setNature(LogicOperator);t->setValue(Advance(2)); break; }
        case    '>' :if(lookAhead(2)[1] == '='){t->setNature(LogicOperator);t->setValue(Advance(2)); break;}
