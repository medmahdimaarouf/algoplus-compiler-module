...
public:
    ...
    std::string toString(){
        ...
        if(hasElse()){
            tostring += "\n     SINON\n";
            for(int i = 0; i < elseSatment->getChildStatments().size();i++)
                tostring += elseSatment->getChildStatments().at(i)->toString();
        }
        return tostring + "\n FIN SI";
    }
private:
...
public:
    ...
    QList<Statment*> * get(){return listToDoStatments;}
    void set(QList<Statment*> * listToDoStatments){this->listToDoStatments = listToDoStatments;}
    void addToDoStatment(Statment * statment){if(!listToDoStatments)listToDoStatments = new QList<Statment*>(); listToDoStatments->append(statment);}
    std::string toString(){
...
private:
    Token * value_case;
    QList<Statment*> * listToDoStatments = NULL;
};
...
public:
    ...
    bool hasElse(){return elseSatment != NULL;}
    bool hasCaseStatments(){return (listCases)?!listCases->isEmpty():false;}
    void setElseStatment(Statment * elseStatment){this->elseSatment = elseStatment;}
    Statment * getElseStatment(){return this->elseSatment;}
    ...
    std::string toString(){
        ...
        if(hasCaseStatments()){
            ...
            for(;i < listCases->size();i++){
                tostring += listCases->at(i)->toString();
            }
        }
...
public:
    ...
    CallProc_Statment(Statment * parent,const std::string idProcedure):Statment(parent) {setIdProcedure(idProcedure);}

    void addEffectifArg(NodeExpression * arg){if(listEffectefArgs == NULL)listEffectefArgs = new QList<NodeExpression*>();listEffectefArgs->append(arg);}

    void setEffectifArgs(QList<NodeExpression*> * listEffectefArgs){this->listEffectefArgs = listEffectefArgs;}

    QList<NodeExpression*> * getEffectifArgs(){return listEffectefArgs;}

    bool hasEffectifArgs(){return listEffectefArgs?!listEffectefArgs->isEmpty():false;}

    std::string getIdProcdure(){return idProcedure;}

    void setIdProcedure(const std::string idProcedure){this->idProcedure = idProcedure;}

    std::string toString(){
        std::string tostring = "\n PROC " + getIdProcdure() + " (";
        if(hasEffectifArgs()){
            int i = 0;
            for(; i < getEffectifArgs()->size() - 1;i++){
                tostring += getEffectifArgs()->at(i)->toString() + ",";
            }
            tostring += getEffectifArgs()->at(i)->toString() + ")";
...
private:
    QList<NodeExpression*> * listEffectefArgs = NULL;
    std::string idProcedure;
};
