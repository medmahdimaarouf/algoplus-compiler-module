...
LexicalAnalyser::LexicalAnalyser()
{
    ...
    listKeys<<"ALGORITHME"<<"CONSTANTES"<<"TYPES"<<"ENREGISTREMENT"<<"FICHIER"<<"TABLEAU"<<"VARIABLES"<<"FONCTION"<<"PROCEDURE"<<"DEBUT"<<"POUR"<<"SI"<<"SELON"<<"TANTEQUE"<<"REPETER"<<"JUSQU_A"<<"FN"<<"NON"<<"ET"<<"OU"<<"MOD"<<"DIV"<<"NIL"<<"NULLE"<<"FIN"<<"DANS"<<"VRAI"<<"FAUX"<<"RETOURNER"<<"FAIRE"<<"SI_NON";
    // - ATTENTION !
}
...
Token * LexicalAnalyser::NextToken()
{
    ...
    if(currentIndex >= 0)
    {
        ...
        token = new Token(Currentlignecode,CurrentColumnlignecode,currentIndex);
...
// --
        ...
        default:
            if(stream[currentIndex].isDigit())
            {
                ...
                {
                    token->setValue(Id());
                    if(!listKeys.contains(QString::fromStdString(token->Value()).toUpper()))
                        token->setNature(ID);
                    else
        ...
        token->setBackToken(currentToken);
        if(currentToken)currentToken->setNextToken(token);
    }
...
Token *LexicalAnalyser::BackToken()
{
    ...
    if(currentToken && lastToken){
        currentIndex = currentToken->getBeginIndex();
    }
    return lastToken;
}
...
{
    this->currentIndex = token->getBeginIndex();
