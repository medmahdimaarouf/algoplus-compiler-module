...
void LexicalAnalyse(){
    ...
    Token *t;
        ...
        while (t != NULL ){
            cout << endl<<"(" << t->getLineNumber() << ":" << t->getColumnNumber() << ")\t Token  : " + spacerSpacers15(t->Value()) + "\t ,Nature :  " <<  t->stringNature() << endl;
            t = lexicalAnalyser.NextToken();
        }
...
void AnalyseExpression(std::string exp_str){
    try{
        ...
        cout << " Pattern expression  : "<< exp_str <<endl<<"\n Analysed expression -> "<< exp->toString()<<endl;
    }
    catch(SyntaxicError error)
    {
        //system("cls");
...
int main(int argc , char ** argv[]){
    ...
    try
    {
         ...
         // + ANALYSE EXPRESSIONS
            //autoExpressionTest();
            AnalyseExpression("(2 * X + 3 * Y + 5 = 0 ET 5 <> V OU BOOL)");
        // - ANALYSE EXPRESSIONS
