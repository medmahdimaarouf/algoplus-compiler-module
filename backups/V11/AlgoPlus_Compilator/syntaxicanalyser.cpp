...
{
	/*
		NOTE BIEN  : POUR LES INSTRUCTIONS ASSIGN VALUE , PROC CALL , FCT RETURN 
		OBTENIR VOTRE PREMIER TOKEN 'T'
		SI 'T' EST UNE ID 
				SI ( 'T' EST EGALE A ID FCT COURANTE ) OU ( 'T' EST EGALE A "RETOURNER" )
					-->ALLER VERS RETURN FCT
				SI NON 
					--> OBTENIR VOTRE DEUXIEME TOKEN 'T2' 
					SI NON SI ( 'T2' EST UN OPERATEUR BITE ) OU ( 'T2' EGALE A "<--" )
						--> ALLER VERS AFFECTAON 
					SI NON SI 'T2' EST UN PARENTHESE OUVERT 
						--> ALLER VERS CALL PROC
					SI NON 
						--> ERREUR "ILLEGAL EXPRESSION FORMAT"
		SI NON
			--> ERREUR "ILLEGAL EXPRESSION FORMAT"
	*/
    bool IN_Si              = false;
    ...
    ReloadAll:{
            ...
            {
                qDebug()<<"Serching instriction behavior "<<t->toString();//<<!IN_Si << !IN_Pour << !IN_TanteQue << !IN_Repeter << !IN_Selon << !IN_returnFct << !IN_CallProc <<!IN_With << !IN_Else;
                ...
                if(toUpper(t->Value()) == FIN || (static_cast<Repeat_Statment*>(parentstatment) && toUpper(t->Value()) == Jusqu_A))
                {
                    return;
                }
                else
                if(t->Nature() == ID)
                {
                    ...
                    if(toUpper(t->Value()) == Retourner)
                    {
                        if(IN_Fct)
                        {
                            ...
                            Error(t,"Intruction illegale : Retour de fonction interdit sous un autre sous programme .");
                    }
                    else
                    if(IN_Fct && toUpper(t->Value()) == id_program)
                    {
                        IN_returnFct  = true;
                        goto return_ftc_stat;
                    }
                    ...
                    Error(t,"Instruction illegale : Instruction from ambigue " + t->Value());
            }
            if(IN_Si)
            {
               if(toUpper(t->Value()) == "NON" && dynamic_cast<IF_Statment*>(parentstatment))
               {
                   IN_Si = false;
                   IN_Else = true;
               ...
               if(IN_Expression)
               {
                   qDebug()<<"IN IF CONDITION "<<t->toString();
                   NodeExpression * condition = AnalyseExpression(lex);
                   t = lex->BackToken();//t = endToken;
                   if_stat->setCondition(condition);

                   IN_Expression = false;
                   IN_Alors = true;
                   qDebug()<<"OUT IF CONDITION "<<t->toString();
                   goto ReloadAll;
               }
               else
               if(IN_Alors)
               {
                   qDebug()<< "IN ALORS IF "<<t->toString();
                  if(toUpper(t->Value()) == Alors)
                  {
                      Instructions(if_stat,lex);
                      IN_Alors = false;
                      IN_End = true;
                      //endToken = lex->NextToken();
                      qDebug()<< "OUT ALORS IF "<<t->toString();
                  }
                  else
                      Error(t,"Instruction illegale : Mot cle 'Alors' attendu ");
               }
               else
               if(IN_End)
               {
                    qDebug()<<"IN END SI "<<t->toString();
                    if(toUpper(t->Value()) == Si)
                    {
                        IN_Si = false;
                        qDebug()<<"OUT END SI "<<t->toString();
                    }
                    else
                        Error(t,"Instruction illeagel : Fin d'instruction attendu 'FIN SI");
               }
            }
            else
            if(IN_Else){
                qDebug()<<"IN ELSE STATEMENT"<<QString::fromStdString(t->Value());
            }
            else
            if(IN_Pour)
            {
                for_inst:{
                   if(IN_Compter)
                   {
                        //qDebug()<<"IN compter"<<t->toString();
                        NodeExpression * compter = AnalyseExpression(lex);
                        t = lex->currentToken;
                        if(compter == NULL)
                            Error(t,"Compteur du boucle FOR attendu ");
                        else
                            if(dynamic_cast<Reference*>(compter) == NULL){
                                Error(t,"Compteur n'est pas de type scalaire");
                            }
                        else
                        {
                   ...
                   if(IN_DE_Key)
                   {
                       //qDebug()<<"IN de key"<<t->toString();
                        if(toUpper(t->Value()) == "DE")
                         {
                            IN_DE_Key = false;
                            IN_Vi = true;
                            //qDebug()<<"OUT de key "<<t->toString();

                            goto ReloadAll;
                         }
                         else
                            Error(t,"Mot cle 'DE' attendu ");
                   }
                   else
                   if(IN_Vi)
                   {
                       //qDebug()<<"IN VI -> "<< t->toString();
                       NodeExpression * vi = AnalyseExpression(lex);
                       t = lex->BackToken();//t = endToken;
                       if(vi == NULL)
                           Error(t,"Valuer initial du boucle FOR attendu ");
                       else
                       {
                           for_stat->setVi(vi);
                           IN_Vi = false;
                   ...
                   if(IN_TO_FOR)
                   {
                       //qDebug()<<"IN_TO_FOR "<<t->toString();
                        if(toUpper(t->Value()) == "A")
                        {
                            ...
                            Error(t,"Mot cle 'A' de boucle 'Pour' attendu");
                   }
                   else
                   if(IN_Vf)
                   {
                       //qDebug()<<"IN VF"<<t->toString();
                       NodeExpression * vf = AnalyseExpression(lex);
                       t = lex->BackToken();//t  = endToken;

                       if(vf == NULL)
                           Error(t,"Valuer initial du boucle 'POUR' attendu ");
                       else
                       {
                           for_stat->setVf(vf);
                           IN_Vf = false;
                   ...
                   if(IN_Faire)
                   {
                        ...
                        if(toUpper(t->Value()) == "FAIRE")
                        {
                            ...
                            //qDebug()<< "OUT Faire"<<t->toString();
                        }
                        else
                            Error(t,"Mot cle 'Faire' de boucle 'Pour' attendu");
                   }
                   else
                   if(IN_Step)
                   {
                       //qDebug()<<"IN pas "<<t->toString();
                       if(toUpper(t->Value()) == "PAS")
                       {
                           IN_Step = false;
                           IN_AssignValue = true;

                           //qDebug()<< "OUT PAS 1"<<t->toString();

                       }
                       else
                       {
                           lex->BackToken();//endToken = t;
                   ...
                   if(IN_AssignValue)
                   {
                       ...
                       if(t->Value() != "=")
                           Error(t,"Mot cle '=' du pas de boucle 'Pour' attendu");
                       else
                       {
                   ...
                   if(IN_StepValue)
                   {
                       //qDebug()<<"IN STEP VALUE "<<t->toString();
                       NodeExpression * stepValue = AnalyseExpression(lex);
                       t = lex->BackToken();//t = endToken;
                       for_stat->setStep(stepValue);
                       if(stepValue == NULL)
                           Error(t,"Valeur du pas du boucle 'Pour' attendu");
                       else
                       {
                           Instructions(for_stat,lex);
                           ...
                           IN_StepValue = false;
                           IN_End = true;
                           //qDebug()<<"OUT STEP VALUE "<<t->toString();
                       }
                   }
                   else
                   if(IN_End)
                   {
                       ...
                       if(toUpper(t->Value()) == Pour)
                       {
                           ...
                           //qDebug()<<"OUT end pour"<<t->toString();
                            ...
                            Error(t,"Instruction illegal : Fin d'instrction attendu 'FIN POUR' ");
                   }
            ...
            if (IN_TanteQue)
            {
                while_inst:{
                    if(IN_Expression)
                    {
                        //qDebug()<<"IN_Condition while " <<t->toString();
                        NodeExpression * condition = AnalyseExpression(lex);
                        t = lex->BackToken();//t = endToken;
                        if(condition == NULL)
                            Error(t,"Condtion de l ' instruction 'TanteQue' attendu");
                        else
                        {
                            while_stat->setCondition(condition);
                            IN_Expression = false;
                            IN_Faire = true;
                            qDebug()<<"OUT Condition while " <<t->toString();
                        }
                    }
                    else
                    if(IN_Faire)
            ...
            if (IN_Repeter)
            {
                repeat_inst:{
                    ...
                    if(IN_Until)
                    {
                        ...
                        t = lex->BackToken();//t = endToken;
                        if(condition == NULL)
                            Error(t,"Condtion de l instruction 'Repeter' attendu");
                        else{
            ...
            if (IN_Selon)
            {
                switch_stat:{
                    if(IN_Selector)
                    {
                        ...
                        t = lex->currentToken;
                        if(!selector)
                           Error(t,"Selecteur de choix multiple attendu ");
                        else
                        {
                    ...
                    if(IN_Case)
                    {
                        ...
                        if(t->Nature() == Reel || t->Nature() == NUMBER || t->Nature() == CHAR || toUpper(t->Value()) == VRAI || toUpper(t->Value()) == FAUX)
                        {
                            case_stat->setCaseValue(t);
                            IN_Case = false;
                        }
                        else
                            Error(t,"La valeur '" + t->Value() + "' n est pas de type scalire pour un choix 'Selon .. Faire'");
                    }
                    else
                    if(IN_AssignType)
                    {
                        if(t->Nature() != AssignType)
                            Error(t,"Mot cle ':' attendu ");
                        else
                        {
                            IN_AssignType = false;
            ...
            if (IN_With)
            {
                with_stat:{
                    if(IN_Selector)
                    {
                        //qDebug()<<"IN with selector "<<t->toString();
                        ...
                        if(select)
                        {
                            ...
                            Error(t,"Instruction illegale : Selecteur d instruction avec attendu ");
                    }
                    else
                    if(IN_Faire)
                    {
                        //qDebug()<< "IN Faire with "<<t->toString();
                        if(toUpper(t->Value()) == Faire)
                        {
                            IN_Faire = false;
                            //endToken = NULL;
                            ...
                            Error(t,"Instrcution illeagle  : Mot lce 'Faire ' attentdu ");
                    }
                    else
                    if(IN_End)
                    {
                        ...
                        if(toUpper(t->Value()) == Avec)
                        {
                            ...
                            Error(t,"Instruction illegale : Fin d instruction AVEC attendu 'FIN AVEC'");
                    }
                }
            }
            else
            if(IN_AssignValue)
            {
                if(IN_Expression)
                {
                    ...
                    if(expression)
                    {
                        ...
                        Error(t,"Instrcution illegale : Valeur de l'affectation attendu ");
                }
            }
        }
    }
...
{
    /**
     *  SI L'EXPRESSION EST PARENTHESE FIRST OP ARG DOIT ETRE UNE (EXPRESSION) ET SECOND ARG UNE SIMPLE EXPRESSION
     **/
    bool IN_Expression              = false;
    ...
    while (t = lex->NextToken()) {
        ...
        ReloadAll:{
            qDebug()<<"HERE RELOAD ALL "<< t->toString() <<endl;
            //if (t->getLineNumber() != endToken->getLineNumber() || t->Nature() == Rp)
            if( !t->isInLigne() || t->Nature() == Rp)
            {
                qDebug()<<"THE SHIT IS HERE;";
                goto EndExpression;
            }
            else
            if (!IN_Expression && !IN_ExpressionOperator && !IN_ExpressionWithSignelOp)
            {
                qDebug()<<"Searching expression direction"<<t->toString();

                if(t->Nature() == Lp && expression == NULL)
                {
                    NodeExpression * ndr = AnalyseExpression(lex);
                    t = lex->currentToken;
                    if(ndr == NULL)
                        Error(t,"Expression illegal : Expression vide '( )' ");
                    else
                    {
                        expression = new Expression(ndr);

                        // + FOR CHECKING
                        if(t){
                            if(t->Nature() != Rp)
                                Error(t,"Expression illegal : Spearateur ' ) ' attendu " );
                        }
                        else
                            Error(t,"Expression illegal : Spearateur ' ) ' attendu a la fin d expression ");
                        // - FOR CHECKING
                ...
                if(expression == NULL && listExSignelOperators.contains(toUpper(t->Value())))
                {
                ...
                if(listExOPerators.contains(toUpper(t->Value())))
                {
                    IN_ExpressionOperator = true;
                ...
                ExpressionOperator:{
                    ...
                    if(listExOPerators.contains(toUpper(t->Value())))
                    {
                        if(!expression->hasExpressionOperator())
                        {
                            ...
                            if(lex->currentToken)t = lex->currentToken;

                            if(operatorExp->SecondOperand == NULL)
                                Error(t," Expression incomplete : deuxieme facteur de l'operateur '" + operatorExp->value->Value() + "' attendu  ");

                            expression->setExpressionOperator(operatorExp);
                            return expression;
                        }
                        else
                            Error(t,"Expression illegal : Duplication d 'operateur d 'expression '" + expression->getExpressionOperator()->value->Value() + "'");
                    }
                    else
                    {
                        EndExpression:{
                            if(expression != NULL)
                            {
                                if(expression->hasExpressionOperator())
                                {
                                    if(expression->getExpressionOperator()->SecondOperand == NULL)
                                        Error(t,"Expression incomplete : deuxieme facteur de l'operateur '" + expression->getExpressionOperator()->value->Value() + "' attendu ");
                                }
                            }
                            return expression;
            ...
            if (IN_Expression)
            {
                _Expression:{
                    ...
                    if (!IN_reference && !IN_CallFunction && !IN_ConstValue)
                    {
                        ...
                        if(t->Nature() == Reel || t->Nature() == NUMBER || t->Nature() == STR || t->Nature() == CHAR || toUpper(t->Value()) == "VRAI" || toUpper(t->Value()) == "FAUX" || toUpper(t->Value()) == "NIL" || toUpper(t->Value()) == "NULLE")
                        {
                            IN_ConstValue = true;
                            goto _ConstValue;
                        }
                        ...
                        {
                            Error(t,"Expression illegal : Premier facteur de l 'expression attendu ");
                        }
                        else
                            Error(t,"Expression illegal : 2eme facteur d'expression attendu' ");
                    }
                    else
                    if (IN_reference)
                    {
                        _Reference:{
                            qDebug()<< "IN_Reference"<<t->toString();
                            if(IN_ByteOperator)
                            {
                                if(t->Nature() == LocationSpecifier)
                                {
                                    dynamic_cast<Reference*>(expression)->setByteOperator(new Reference::ByteOperator(t));
                                }
                                else
                                if(t->Nature() == LI)
                                {
                                    Reference::ByteOperator * aux = new Reference::ByteOperator(t);
                                    ...
                                    if(index)
                                    {
                                        ...
                                        {
                                            if(t->Nature() != RI)
                                                Error(t,"Reference illegal : Mot ']' attendu");
                                           // t = lex->currentToken;
                                        }
                                        else
                                            Error(t," Expression incomplete : Mot ']' attendu au fin de l'expression ");
                                    }
                                    else
                                        Error(t,"Expression illegale : Argument de l'operateur bite '[]' atttendu ");
                                ...
                                {
                                    Reference::ByteOperator * aux = new Reference::ByteOperator(t);
                                    Token * errorToken = t; // TRY/CHECK TO ELIMINATE THAT FUCKING SOON ( 10 LIGNES (WITHOUT BLANKS ) AFTER ) .
                                    t = lex->NextToken();

                                    if(t)
                                    {
                                        if(t->Nature() == ID)
                                            aux->setArgment(new Reference(t));
                                        else
                                            Error(t,"Reference illegale : Argument de loperateur bite '.' incorrecte ");
                                    }
                                    else
                            ...
                            if(IN_IDref)
                            {
                                if(t->Nature() == ID)
                                {
                                    expression = new Reference(t);
                                    IN_IDref = false;
                                    IN_ByteOperator = true;
                                }
                                else
                                    Error(t,"Expression illegale : reference attendu ");
                            }
                        }
                    }
                    else
                    if (IN_CallFunction)
                    {
                        CallFunction:{
                            ...
                            if(IN_id_fct)
                            {
                                if(t->Nature() == ID)
                                {
                                    expression = new FctCall(t->Value());
                                    IN_id_fct = false;
                                    IN_Lp = true;
                                }
                                else
                                    Error(t,"Expression illegal : Identificateur de fonction a apeller attendu");
                            }
                            else
                            if(IN_Lp)
                            {
                                ...
                                if(t->Nature() == Lp)
                                {
                                    ...
                                    Error(t,"Expression illegal : '(' attendu ");
                            }
                            else
                            if(IN_Rp)
                            {
                                if(t)
                                {
                                    ...
                                    if(dynamic_cast<FctCall*>(expression)->hasEffectArgs() && t->Value() == ",")
                                        Error(t,"Expression illegal : Un argument effective requis ");
                                    else
                                        Error(t,"Expression illegal : ')' attendu ");
                            ...
                            if(IN_EffectefArgs)
                            {
                                qDebug()<<"IN_EffectefArgs "<<t->toString();
                                ...
                                ListEfArgs:
                                {
                                    do{
                                        ...
                                        if(dynamic_cast<FctCall*>(expression)->hasEffectArgs())
                                            Error(t,"Expression illegal : Argument effective requis ");
                                        else
                                        {
            ...
            if(IN_ExpressionWithSignelOp)
            {
                ExpressionWithSignelOp:{
                    ...
                    if(node)
                    {
                        return new ExpressionWithSignelOperator(node,opval);
                    }
                    else
                        Error(t,"Argmant de loperateur ' " + opval->Value() + "' attendu ");
